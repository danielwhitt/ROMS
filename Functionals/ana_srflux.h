...
!
 ...
 || defined DIURNAL_CYCLE_DAN
      integer :: iday, month, year
      real(r8) :: Dangle, Hangle, LatRad
      real(r8) :: cff1, cff2, hour, yday
# ifdef ALBEDO || defined DIURNAL_CYCLE_DAN
...
# if defined UPWELLING
      DO j=JstrT,JendT
        ...
        END DO
      END DO
# elif defined GEOSTROPHIC3D
      DO j=JstrT,JendT
        DO i=IstrT,IendT
!          srflx(i,j)=cff*0.0_r8
!          srflx(i,j)=cff*367.616_r8
          cff=1.0_r8/(rho0*cp)
          srflx(i,j)=cff*158.075_r8
#ifdef DIURNAL_CYCLE_DAN
! wait 4 days to turn on the diurnal cycle
      IF (tdays(ng).gt.0.1_r8) THEN
! calculate declanation angle for yearday 183 
        Dangle=23.44_r8*COS((172.0_r8-183.0_r8)*2.0_r8*pi/365.25_r8)
        Dangle=Dangle*deg2rad
        Hangle=(12.0_r8-MOD(tdays(ng),1.0_r8)*24.0_r8)*pi/12.0_r8
! fixed latitude 30.0 N
        LatRad=30.0_r8*deg2rad
        cff1=SIN(LatRad)*SIN(Dangle)
        cff2=COS(LatRad)*COS(Dangle)
! fixed longitude of 0.0
        cff=(cff1*ACOS(-cff1/cff2)+SQRT(cff2*cff2-cff1*cff1))/pi
        srflx(i,j)=MAX(0.0_r8,                                          &
     &                     srflx(i,j)/cff*                              &
     &                     (cff1+cff2*COS(Hangle-0.0_r8*deg2rad)))
      END IF
#endif
        END DO
      END DO
