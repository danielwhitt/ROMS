...
!
      ...
      real(r8) :: cff, cff1, cff2, cff3, cff4, cff5, dtdays
...
!
!  DAN ADDED Mayzaud and Poulet 1978 grazing formulation
...
!  (ZooMRD rate).
!  DAN ADDED QUADRATIC MORTALITY TO ZOOPLANKTON
          cff3=dtdays*ZooMRD(ng)
          cff2=dtdays*ZooMRN(ng)
!          cff1=1.0_r8/(1.0_r8+cff2+cff3)
          DO k=1,N(ng)
            DO i=Istr,Iend
! SET THE QUADRATIC MORTALITY RATE HERE:
              cff5=dtdays*0.28_r8*Bio(i,k,iZoop)
              cff1=1.0_r8/(1.0_r8+cff2+cff3+cff5)
              Bio(i,k,iZoop)=Bio(i,k,iZoop)*cff1
!              Bio(i,k,iZoop)=Bio(i,k,iZoop)                             &
!     &                     -cff5*Bio(i,k,iZoop)**2.0_r8
              Bio(i,k,iNO3_)=Bio(i,k,iNO3_)+                            &
     &                       Bio(i,k,iZoop)*cff2
              Bio(i,k,iSDet)=Bio(i,k,iSDet)+                            &
     &                       Bio(i,k,iZoop)*(cff3+cff5)
!              Bio(i,k,iSDet)=Bio(i,k,iSDet)+                            &
!     &                       cff5*Bio(i,k,iZoop)
            END DO
          END DO
...
!
     ...
     &                       Bio(i,k,iSDet)*cff2
          ...
          END DO
!
! Deep N restoring in bottom 3 grid cells DAN ADDED
!
          DO k=1,3
            DO i=Istr,Iend
! set restoring timescale and deep N concentration here
              Bio(i,k,iNO3_)=Bio(i,k,iNO3_)-                            &
     &          dtdays*0.25_r8*(Bio(i,k,iNO3_)-2.5_r8)
            END DO
          END DO
